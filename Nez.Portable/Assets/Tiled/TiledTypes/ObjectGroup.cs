...
namespace Nez.Tiled
{
    ...
    public class TmxObject : ITmxElement
    {
        ...
        public TmxObject(TmxMap map, XElement xObject)
        {
            ...
            X = (float?)xObject.Attribute("x") ?? 0.0f;
            Y = (float?)xObject.Attribute("y") ?? 0.0f;
            ...
            if (template != null)
            {
                Template = new TmxObjectTemplate(map, template.Value);

                if (xObject.Attribute("type") == null)
                {
                    Type = Template.TemplateObject.Type;
                }
                if (xObject.Attribute("width") == null)
                {
                    Width = Template.TemplateObject.Width;
                }
                if (xObject.Attribute("height") == null)
                {
                    Height = Template.TemplateObject.Height;
                }
            }
    ...
    public class TmxObjectTemplate : TmxDocument
    {
        ...
        public TmxObjectTemplate(TmxMap map, string source)
        {

            Source = Path.Combine(map.TmxDirectory, source);

            XDocument document = ReadXml(Source);
            var xTemplate = document.Element("template");

            TemplateObject = new TmxObject(map, xTemplate.Element("object"));
        }
