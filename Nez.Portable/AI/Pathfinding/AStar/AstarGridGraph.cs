...
namespace Nez.AI.Pathfinding
{
    ...
    public class AstarGridGraph : IAstarGraph<Point>
    {
        ...
        public AstarGridGraph(int width, int height, int minX, int minY) : this(width, height)
        {
            _minX = minX;
            _minY = minY;
        }
        ...
        {
            return _minX <= node.X && node.X < (_minX + _width) && _minY <= node.Y && node.Y < (_minY + _height);
        }
